...
class AlexNet(Model):
    ...
    def create(self):
        ...
        conv1 = Model.conv(self.tensor, 11, 11, 96, 4, 4, padding='VALID', name='conv1')
        ...
        conv2 = Model.conv(pool1, 5, 5, 256, 1, 1, groups=2, name='conv2')
        ...
        conv3 = Model.conv(pool2, 3, 3, 384, 1, 1, name='conv3')
        
        # 4th Layer: Conv (w ReLu) splitted into two groups
        conv4 = Model.conv(conv3, 3, 3, 384, 1, 1, groups=2, name='conv4')
        
        # 5th Layer: Conv (w ReLu) -> Pool splitted into two groups
        conv5 = Model.conv(conv4, 3, 3, 256, 1, 1, groups=2, name='conv5')
        ...
        fc6        = Model.fc(pool5_flat, pool5_out, 4096, name='fc6')
        dropout6   = tf.nn.dropout(fc6, self.keep_prob)
        
        # 7th Layer: FC (w ReLu) -> Dropout
        fc7      = Model.fc(dropout6, 4096, 4096, name='fc7')
        dropout7 = tf.nn.dropout(fc7, self.keep_prob)
    
        # 8th Layer: FC and return unscaled activations (for tf.nn.softmax_cross_entropy_with_logits)
        self.final = Model.fc(dropout7, 4096, self.num_classes, relu=False, name='fc8')
